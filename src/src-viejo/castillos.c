...
void asignar_g(pcastillo_t c,int g){//asigna defensores del castillo
	if (c != NULL)
		c->g = g;
}
pcastillo_t cargar_castillo(pcastillo_t c,int nro_castillo){
	...
	if((cadena = readline())==NULL){ //leo la cadena de stdin
		#ifdef DEBUG
...
void imprimir_castillo(pcastillo_t c){
	if (c != NULL)
		fprintf(stdout,"Castillo nro:%d a:%d m:%d g:%d\n",c->nro,c->a,c->m,c->g);
}
